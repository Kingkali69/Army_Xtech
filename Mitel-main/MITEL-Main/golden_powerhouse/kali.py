#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# LINUX_MASTER.py - Unified GhostHUD master using unified engine
...
try:
    ...
    pm = None
    print(f"[!] Plugin manager unavailable: {_pm_exc}")

# HTML - All-in-one file - EXACT UI PRESERVED
HTML = """<!DOCTYPE html>
<html>
<head>
...
if __name__ == "__main__":
    # UTF-8 encoding is now handled by ghostops_adapter.py
    ...
    try:
        ...
        # Create master engine (auto-detect platform)
        engine = GhostOpsAdapter.create_master_adapter(platform=None, port=7890)

        # HTML provider function
        def get_html():
            return HTML

        # Start master server with browser auto-open
        GhostOpsAdapter.start_master_server(engine, get_html, auto_open_browser=True)
    except KeyboardInterrupt:
        print("[+] Shutting down")
    except Exception as e:
        print(f"[!] Error: {e}")
