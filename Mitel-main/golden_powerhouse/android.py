#!/usr/bin/env python3
# ANDROID PEER - Unified GhostHUD peer using unified engine
...
# Embedded HTML (full feature set - same as Linux master)
HTML = """<!DOCTYPE html>
<html>
<head>
...
if __name__ == "__main__":
    try:
        ...
        master_ip = None
        master_port = 7890
        local_port = 7892  # Android peer listens here
        ...
        engine = GhostOpsAdapter.create_peer_adapter(platform=None, master_ip=master_ip, local_port=local_port)
        ...
        # Start peer server
        print(f"[+] Starting Android Peer server on port {local_port}...")
        print(f"[+] HTML loaded: {len(HTML)} characters")

        GhostOpsAdapter.start_peer_server(engine, get_html, auto_open_browser=False)
    except KeyboardInterrupt:
        print("[+] Shutting down")
    except Exception as e:
        print(f"[!] Error: {e}")
